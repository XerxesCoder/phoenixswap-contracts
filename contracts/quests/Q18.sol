...
interface IPhoenixLaunchpad {
  function getUserPresaleIds(
    address user
  ) external view returns (uint256[] memory);
}

contract PhoenixSyndicate is Ownable {
  using SafeMath for uint256;
  IPhoenixWorld public immutable phoenixWorld;
  IPhoenixLaunchpad public immutable phoenixLaunchPad;
  bool public isQuestActive = true;
  uint256 public numberPoints = 30;
  uint256 public questId = 18;
  mapping(address => bool) public hasClaimed;

  constructor(address _world, address _launchpad) Ownable(msg.sender) {
    phoenixWorld = IPhoenixWorld(_world);
    phoenixLaunchPad = IPhoenixLaunchpad(_launchpad);
  }
  ...
  function getUserPresaleIdsLength(
    address user
  ) internal view returns (uint256) {
    uint256[] memory presaleIds = phoenixLaunchPad.getUserPresaleIds(user);
    return presaleIds.length;
  }
  ...
  function _canClaim(address _userAddress) internal view returns (bool) {
    if (hasClaimed[_userAddress] || !isQuestActive) {
      ...
      if (!isActiveUser(_userAddress)) {
        return false;
      } else if (getUserPresaleIdsLength(_userAddress) >= 5) {
        return true;
      } else {
        return false;
      }
