...
contract PresalePioneer is Ownable {
    ...
    function getUserPresaleIdsLength(address user)
        internal
        view
        returns (uint256)
    {
    ...
    function _canClaim(address _userAddress) internal view returns (bool) {
        if (hasClaimed[_userAddress] || !isQuestActive) {
            ...
            if (!isActiveUser(_userAddress)) {
                return false;
            } else if (getUserPresaleIdsLength(_userAddress) >= 1) {
                return true;
            } else {
                return false;
            }
